...
const setupSQL = `
-- ===================================================================
-- BuzyBees Forgot Password Complete System Setup
-- ===================================================================
...
BEGIN
  ...
  WHERE password_reset_otps.email = LOWER(user_email)
    ...
    AND password_reset_otps.expires_at > NOW()
    AND (password_reset_otps.is_used = FALSE OR password_reset_otps.is_used IS NULL)
    AND password_reset_otps.verified_at IS NULL
...
BEGIN
  ...
  WHERE password_reset_otps.email = LOWER(user_email)
    ...
    AND password_reset_otps.verified_at IS NOT NULL
    AND password_reset_otps.expires_at > NOW()
    AND (password_reset_otps.is_used = FALSE OR password_reset_otps.is_used IS NULL)
  ORDER BY password_reset_otps.verified_at DESC
  LIMIT 1;
...
serve(async (req) => {
  console.log('🐝 BuzyBees Complete Forgot Password Function called');
      ...
      case 'send_reset_email':
        ...
        try {
          ...
          const { data: emailData, error: emailError } = await resend.emails.send({
            ...
            to: [email],
            subject: `🐝 BuzyBees Password Reset - Code: ${generatedOTP}`,
            html: htmlContent,
          });
