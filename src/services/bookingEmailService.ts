...
class BookingEmailService {
  ...
  async sendBookingConfirmation(emailData: BookingEmailData): Promise<EmailResult> {
    try {
      console.log('📧 Sending booking confirmation email:', emailData.to_email);

      const emailTemplate = this.generateBookingConfirmationTemplate(emailData);
      ...
      const result = await this.sendEmail({
        ...
        html: emailTemplate,
        text: this.generatePlainTextConfirmation(emailData)
      });
  ...
  async sendBusinessNotification(notificationData: BusinessNotificationData): Promise<EmailResult> {
    try {
      ...
      const result = await this.sendEmail({
        ...
        html: emailTemplate,
        text: this.generatePlainTextBusinessNotification(notificationData)
      });
