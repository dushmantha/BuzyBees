...
class AuthServiceClass {
  ...
  async signUp(userData: UserData): Promise<AuthResponse> {
    try {
      ...
      // 3. Create user profile using the database function
      try {
        console.log('📝 Creating user profile with database function...');
        ...
        // Prepare provider data if needed
        const providerData = userData.accountType === 'provider' ? {
        ...
        // Prepare consumer data if needed
        const consumerData = userData.accountType === 'consumer' ? {
          budget_range: 'Not specified',
          location_preference: `${userData.location.city}, ${userData.location.state}`
        } : null;
        ...
        const { data: profileResult, error: profileError } = await supabase
          .rpc('create_user_profile', {
            ...
            p_address: userData.address.trim(),
            p_account_type: userData.accountType,
            p_location_data: locationData,
            p_provider_data: providerData,
            p_consumer_data: consumerData
          });
        ...
        console.log('✅ User profile created successfully with function');
  ...
  /**
   * Sign in user - Enhanced with debugging
   */
...
class LocationServiceClass {
  ...
  async searchLocations(query: string): Promise<LocationSuggestion[]> {
    try {
      ...
      // Mock results - replace with real geocoding service
      const mockResults: LocationSuggestion[] = [
        {
          id: `${query}-1`,
          description: `${query}, New Zealand`,
          latitude: -36.8485,
          longitude: 174.7633,
          city: query,
          state: 'Auckland',
