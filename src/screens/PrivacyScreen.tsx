...
const PrivacyScreen = ({ navigation }: { navigation: any }) => {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const response = await mockPrivacyAPI.deleteAccount('user-123');
              if (response.success) {
  ...
  const handleDownloadData = async () => {
    Alert.alert(
      'Download Data',
      'We will prepare your data for download. You will receive an email with the download link within 24 hours.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const response = await mockPrivacyAPI.downloadData('user-123');
              if (response.success) {
                Alert.alert('Request Submitted', 'Your data download request has been submitted. You will receive an email with the download link within 24 hours.');
              } else {
