...
BEGIN
    -- Create initial staff record (shop owner as default staff)
    IF NOT EXISTS (SELECT 1 FROM shop_staff WHERE shop_staff.shop_id = NEW.id AND shop_staff.provider_id = NEW.provider_id) THEN
        ...
        -- Add staff ID to provider_businesses.staff_ids array
        UPDATE provider_businesses 
        SET staff_ids = array_append(staff_ids, new_staff_id)
        WHERE id = NEW.id;
    END IF;
    
    -- Create initial service (basic service offering)
    IF NOT EXISTS (SELECT 1 FROM shop_services WHERE shop_services.shop_id = NEW.id) THEN
        INSERT INTO shop_services (
            ...
            NEW.id,
            'Basic ' || NEW.category || ' Service',
            'Our standard service offering. Please contact us for more details.',
            100.00,
            60,
            NEW.category,
            'in_house',
        ...
        -- Add service ID to provider_businesses.service_ids array
        UPDATE provider_businesses 
...
BEGIN
    ...
    -- Report results
    IF staff_count > 0 AND services_count > 0 AND discounts_count > 0 THEN
        RAISE NOTICE 'SUCCESS: Shop created with related records - Staff: %, Services: %, Discounts: %', 
                     staff_count, services_count, discounts_count;
    ELSE
        RAISE EXCEPTION 'ERROR: Related records not created - Staff: %, Services: %, Discounts: %', 
                        staff_count, services_count, discounts_count;
    END IF;
